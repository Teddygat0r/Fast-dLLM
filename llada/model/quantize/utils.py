...
def post_rotate_quant_temporary(model, args):
    if args.let:
        ...
        if type(model) != QuantLLadaDecoderLayer:
            ...
            post_fcs_temporary([model.q_proj, model.k_proj, model.v_proj], model.qkv_post_scale)
            post_fcs_temporary([model.up_proj,model.ff_proj], model.fc1_post_scale)
            post_fcs_temporary(model.ff_out, model.down_post_scale)
            post_fcs_temporary(model.attn_out, model.out_post_scale)
    ...
    if args.smooth:
        ...
        if isllama:
            if type(model) != QuantLLadaDecoderLayer:
                ...
                smooth_ln_fcs_temporary(model.attn_norm,[model.q_proj, model.k_proj, model.v_proj],
                                        model.qkv_smooth_scale,model.qkv_smooth_shift)
                smooth_ln_fcs_temporary(model.ff_norm,[model.up_proj,model.ff_proj],
                                        model.fc1_smooth_scale,model.fc1_smooth_shift)
                smooth_fc_fc_temporary(model.up_proj,model.ff_out,
                                    model.down_smooth_scale, model.down_smooth_shift)
                smooth_fc_fc_temporary(model.v_proj,model.attn_out,
                                    model.out_smooth_scale, model.out_smooth_shift)
                smooth_q_k_temporary(model.q_proj, model.k_proj,
                                    model.qkt_smooth_scale)
                model.ff_out.temp_weight = model.ff_out.weight
        else:
...
def smooth_and_let_inplace(model, args):
    if args.smooth:
        ...
        if type(model) != QuantLLadaDecoderLayer:
            ...
            smooth_ln_fcs_inplace(model.post_attention_layernorm,[model.mlp.up_proj,model.mlp.gate_proj], model.fc1_smooth_scale,model.fc1_smooth_shift)
            ...
            smooth_ln_fcs_inplace(model.attn_norm, [model.q_proj, model.k_proj, model.v_proj], model.qkv_smooth_scale, model.qkv_smooth_shift)
            smooth_ln_fcs_inplace(model.ff_norm,[model.up_proj,model.ff_proj], model.fc1_smooth_scale,model.fc1_smooth_shift)
            smooth_fc_fc_inplace(model.up_proj,model.ff_out,
                                model.down_smooth_scale, model.down_smooth_shift)
            smooth_fc_fc_inplace(model.v_proj,model.attn_out,
                                model.out_smooth_scale, model.out_smooth_shift)
            smooth_q_k_inplace(model.q_proj, model.k_proj,
                                model.qkt_smooth_scale)

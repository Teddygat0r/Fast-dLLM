"""
W4A8 Quantization script for LLaDA Models

This script applies INT4 weight and INT8 activation quantization to LLaDA models.
It can work with either the base model or a SmoothQuant-preprocessed model.

Usage:
    # Quantize base model
    python quantize_llada_w4a8.py --model-path GSAI-ML/LLaDA-8B-Instruct
    
    # Quantize SmoothQuant-preprocessed model
    python quantize_llada_w4a8.py --model-path GSAI-ML/LLaDA-8B-Instruct --load-smoothed --smoothed-model-path models/llada_smoothquant.pt
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Quantize LLaDA model to W4A8")
    ...
    print(f"\n[Step 3/5] Quantizing model: weights=qint4, activations=qint8")
    print(f"  Excluding layers: {args.exclude_layers}")
    ...
    quantize(model, weights=qint4, activations=qint8, exclude=args.exclude_layers)
